...
chunks_cleaned = pd.read_csv('StockUniteLegale_cleaned.csv', chunksize=chunk_size, low_memory=False)
...
template_data = pd.read_csv('template.csv', delimiter=';')

# Merge the matched data with the template data on 'siren_number' and 'siren'
merged_data = template_data.merge(matched_data, left_on='siren_number', right_on='siren', how='left')
