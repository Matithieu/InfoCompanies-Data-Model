...
def extract_phone_number(soup):
    ...
    phone_element = soup.find("span", attrs={"aria-label": lambda x: x and 'Appeler le' in x})
...
def extract_schedule(soup):
    ...
    for table in tables:
        day_column = table.find_all("td", string=lambda x: x and x.strip().lower() in dayOfTheWeek)
...
def extract_instagram(soup):
    instagram = ""
    instagram_element = soup.find("a", href=lambda x: x and 'https://www.instagram.com/' in x)
...
def extract_facebook(soup):
    facebook = ""
    facebook_element = soup.find("a", href=lambda x: x and 'https://www.facebook.com/' in x)
    if facebook_element:
...
def extract_linkedin(soup):
    linkedin = ""
    linkedin_element = soup.find("a", href=lambda x: x and 'https://www.linkedin.com/' in x)
    if linkedin_element:
...
def extract_youtube(soup):
    youtube = ""
    youtube_element = soup.find("a", href=lambda x: x and 'https://www.youtube.com/' in x)
    if youtube_element:
